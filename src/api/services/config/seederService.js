    ...
    async seedingSeeds() {
        try {
            ...
            if (!seeder_data || !seeder_data.length) {
                utilService.warnLog('Seeder service has no data to seeding...!');
                return;
            }
            ...
            await Promise.all(_.map(seeder_data, async (seederItem) => {
                ...
                let data = JSON.parse(fs.readFileSync(path.join(global.appPath, 'src', 'seeder-data', `${fileName}.json`), 'utf8'));
                ...
                await Promise.all(_.map(data, async (record) => {
                    try {
                        ...
                        utilService.errLog(`Error while seed(saving) record: ${e}`);
                    }
                }));
            ...
            utilService.errLog(`Error while seeding: ${e}`);
        }
    },
