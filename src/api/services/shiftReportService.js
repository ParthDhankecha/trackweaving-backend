...
async function sendShiftReportForWorkspace(workspace, shiftType) {
    const reportUsers = await userService.findV2({
    ...
    if (!reportUsers.length) {
        utilService.log(`Shift report skipped for workspace ${workspace.firmName}: no users with WhatsApp reports enabled.`);
        return;
    }
    ...
    if (!machines.length) {
        utilService.log(`Shift report skipped for workspace ${workspace.firmName}: no machines found.`);
        return;
    }
    ...
    if (!exportData.list.length || !exportData.list.some(item => item.list?.length)) {
        utilService.log(`Shift report skipped for workspace ${workspace.firmName}: no report data for ${shiftLabel}.`);
        return;
    }
    ...
    try {
        ...
        sendResults.forEach((result, index) => {
            ...
            if (result.status === 'fulfilled') {
                utilService.log(`Shift report sent to ${result.mobile} (${userName}) for workspace ${workspace.firmName} (${shiftLabel}).`);
                return;
            }
            utilService.errLog(`Failed to send shift report to ${result.mobile} (${userName}) for workspace ${workspace.firmName}: ${result.error?.message || 'Unknown error'}`);
            if (result.error) {
                utilService.log(result.error);
            }
        });
    } finally {
...
module.exports = {
    async sendShiftReports(shiftType) {
        const shiftLabel = getShiftLabel(shiftType);
        utilService.log(`Starting automatic ${shiftLabel} report job...`);

        if (!whatsappService.isEnabled()) {
            utilService.errLog('WhatsApp is not configured. Automatic shift reports will not be sent.');
            return;
        }
        ...
        for (const workspace of workspaces) {
            try {
                ...
                utilService.errLog(`Failed to send shift report for workspace ${workspace.firmName}: ${error.message}`);
                utilService.log(error);
            }
        }

        utilService.log(`Automatic ${shiftLabel} report job completed.`);
    },
